...
class CommitView(
    ...
    private fun updateUI() {
        ...
        if(authService.isUserLoggedIn()){
            ...
            val commitProjectHolder = Anchor("/project?p=${currentCommit!!.project_id}",
                commitProject!!.name
            ).apply {
                style.set(CSS.OVERFLOW_WRAP, OVERFLOW_WRAP.BREAK_WORD)
                style.set(CSS.MARGIN_BOTTOM, 10.px)
            ...
            try{
                ...
                val filesGrid = Grid<File>().apply {
                    ...
                    addItemClickListener { event ->
                        ...
                        val filetype = processFile(fileT)
                        val dialog = com.vaadin.flow.component.dialog.Dialog()
                        when {
                            filetype.isNullOrEmpty() -> {
                                showWarning("Непідтримуваний формат, завантажте файл")
                            }
                            filetype == "text" -> {
                                val b =Button().apply {
                                    icon = Icon(VaadinIcon.DOWNLOAD)
                                ...
                                val fileToDownload = java.io.File(event.item.file_path)
                                val buttonWrapper = FileDownloadWrapper(
                                    StreamResource(fileToDownload.name, InputStreamFactory {ByteArrayInputStream(fileToDownload.readBytes())})
                                )
                                buttonWrapper.wrapComponent(b)
                                ...
                                dialog.add(buttonWrapper, TextArea().apply {
                                    value = fileData
                                    setSizeFull()
                                    isReadOnly = true
                                })
                ...
                showWarning("Даний внесок не має файлів")
                dynamicLayout.add(
                    HorizontalLayout(
                        leftBarHolder
                    ).apply {}
